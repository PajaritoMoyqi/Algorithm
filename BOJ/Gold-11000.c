...
int main( void )
{
    ...
    PQNode* PQNodeSet = (PQNode*)malloc( sizeof( PQNode ) * n );

    int i;
    for ( i = 0; i < n; i++ )
        scanf( "%d%d", &PQNodeSet[i].Data, &PQNodeSet[i].Priority );

    qsort( PQNodeSet, n, sizeof( PQNode ), cmp );
    // for ( i = 0; i < n; i++ )
    //     printf( "%d ", PQNodeSet[i].Priority );
    // puts( "" );
    ...
    for ( i = 0; i < n; i++ )
    {
        PQ_Enqueue( PQ, PQNodeSet[i] );

        PQ_Dequeue( PQ, &Popped );

        // printf( "Popped: %d ~ %d, Candidate: %d ~ %d\n", Popped.Data, Popped.Priority, PQNodeSet[i].Data, PQNodeSet[i].Priority );

        if ( Popped.Priority > PQNodeSet[i].Data )
