...
int main( void )
{
    ...
    PQueue* PQSmaller = Create( n );
    PQueue* PQBigger = Create( n );
    ...
    for ( i = 0; i < n; i++ )
    {
        ...
        if ( !PQSmaller->UsedSize || num <= PQSmaller->Nodes[0].Data )
        {
            EnqueueMax( PQSmaller, num );
            if ( PQSmaller->UsedSize > PQBigger->UsedSize + 1 )
            {
                PQNode* Popped = DequeueMax( PQSmaller );
                Enqueue( PQBigger, Popped->Data );
            }
        }
        else if ( !PQBigger->UsedSize || num >= PQBigger->Nodes[0].Data )
        {
            Enqueue( PQBigger, num );
            if ( PQBigger->UsedSize >= PQSmaller->UsedSize + 1 )
            {
                PQNode* Popped = Dequeue( PQBigger );
                EnqueueMax( PQSmaller, Popped->Data );
            }
        }
        else
        {
            if ( PQSmaller->UsedSize <= PQBigger->UsedSize ) 
            {
                EnqueueMax( PQSmaller, num );
                if ( PQSmaller->UsedSize > PQBigger->UsedSize + 1 )
                {
        ...
        printf( "%d\n", PQSmaller->Nodes[0].Data );

        // int j;
        // printf( "PQSmaller: " );
        // for ( j = 0; j < PQSmaller->UsedSize; j++ )
        //     printf( "%d ", PQSmaller->Nodes[j].Data );
        // puts( "" );
        // printf( "PQBigger: " );
        // for ( j = 0; j < PQBigger->UsedSize; j++ )
        //     printf( "%d ", PQBigger->Nodes[j].Data );
        // puts( "" );
    }
